...
#[test]
fn simple_retries() -> Result<(), StdErrorType> {
...
#[test]
fn zero_cost_abstractions() -> Result<(), StdErrorType> {
    ...
    let result = socket.send(MyPayload { message: case_name });
    assert_eq!(result, expected, "In '{}'", case_name);
    assert_eq!(socket.is_connected, true, "In '{}'", case_name);

    let case_name = "2) Failed fatably at the first shot";
    println!("\n{}:", case_name);
    let expected = Err(SendingErrors::QuotaExhausted { payload: None, root_cause: format!("any root cause....").into() });
    ...
    socket.connect_to_server()?;
    let result = socket.send(to_send());
    assert_eq!(result, expected, "In '{}'", case_name);
    assert_eq!(socket.is_connected, false, "In '{}'", case_name);
...
struct Socket {
    ...
    sending_fatal_failure_latch_countdown:    i32,
    sending_attempts:                         u32,
}
...
impl Socket {
    ...
    pub fn new(connection_max_retries:                   u32,
               ...
               sending_max_retries:                      i32,
               sending_success_latch_countdown:          i32,
               sending_fatal_failure_latch_countdown:    i32) -> Self {
        Self {
            is_connected: false,
            connection_max_retries,
            connection_success_latch_countdown,
            connection_fatal_failure_latch_countdown,
            connection_attempts: 0,
            sending_max_retries,
            sending_success_latch_countdown,
            sending_fatal_failure_latch_countdown,
            sending_attempts: 0,
         }
    }

    pub fn connect_to_server(&mut self) -> Result<(), ConnectionErrors> {
        self.connect_to_server_retry()
            .retry_with(|_| self.connect_to_server_retry())
    ...
    fn send<T: Debug + PartialEq>
        ...
        self.send_retry(payload)
            ...
            .retry_with(|(payload, retry_start)| {
                let loggable_payload = format!("{:?}", &payload);
                if !self.is_connected() {
                    if let Err(err) = self.connect_to_server() {
    ...
    fn connect_to_server_retry(&mut self) -> RetryConsumerResult<(), (), ConnectionErrors> {
    ...
    fn connect_to_server_raw(&mut self) -> Result<(), ConnectionErrors> {
        self.connection_success_latch_countdown -= 1;
        self.connection_fatal_failure_latch_countdown -= 1;
        self.connection_attempts += 1;
        if self.connection_success_latch_countdown == 0 {
            self.is_connected = true;
            Ok(())
        } else if self.connection_fatal_failure_latch_countdown == 0 {
            self.is_connected = false;
            Err(ConnectionErrors::WrongCredentials)
        } else if self.connection_success_latch_countdown < 0 {
            self.is_connected = true;
            Ok(())
        } else {
            self.is_connected = false;
    ...
    fn send_raw<T: Debug + PartialEq>
        ...
        if !self.is_connected {
            ...
            self.sending_success_latch_countdown -= 1;
            self.sending_fatal_failure_latch_countdown -= 1;
            self.sending_attempts += 1;
            if self.sending_success_latch_countdown == 0 {
                Ok(())
            } else if self.sending_fatal_failure_latch_countdown == 0 {
                self.is_connected = false;
